  ...
  function(fit, type, method, join, notes, postfit)
{
  ...
  mmeth <- fit$method
  mcont <- coxph.control()
#  if(inherits(fit, "mfp")) varnames <- names(fit$fit$coefficients) else                  ### noch offen
    varnames <- names(fit$coefficients) 
    
  if(!is.null(join)) {                                                           # for rcs
    join2 <- sapply(join, function(x) sub(x = x, pattern = "rcs(", replacement = "", 
                                          fixed = TRUE), simplify = FALSE)    
    for (i in 1L:length(join)) {
      comp <- join2[[i]] == join[[i]]
      join2[[i]][!comp] <- sub(join2[[i]][!comp], pattern = ")", replacement = "", fixed = TRUE)
      join2[[i]][join[[i]] %in% varnames] <- join[[i]][join[[i]] %in% varnames]   # if too much was remoded
    }
    ...
    if(rcs.test[[1L]] && any(unlist(join) != unlist(join2))) {
      ...
      if(!all(unlist(join2) %in% unique(varnames2))) { 
